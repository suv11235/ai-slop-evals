...
def openrouter_chat_completion(
    *,
    ...
    timeout_s: int = 120,
    max_retries: int = 4,
    operation_name: str = "completion",
    session_id: str | None = None,
) -> str:
    with tracer.start_as_current_span(f"openrouter.{operation_name}") as span:
        span.set_attribute(
            SpanAttributes.OPENINFERENCE_SPAN_KIND,
            OpenInferenceSpanKindValues.LLM.value,
        )
        span.set_attribute(SpanAttributes.INPUT_VALUE, json.dumps(messages, ensure_ascii=False))
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    error_payload = ""
                last_err = RuntimeError(f"HTTP {e.code} {e.reason}: {error_payload[:500]}")
...
def build_cgf_messages(
    *,
    round_n: int,
    prior_fgf: dict | None,
    scenario: dict = SCENARIO_S1_BAIL,
) -> list[dict]:
    system = (
...
def build_fgf_messages(
    *,
    ...
    user = (
        scenario["fgf_background"]
        + "\n\n"
        + "Patch menu (choose one for CGF to try next):\n- "
        + "\n- ".join(scenario["patch_menu"])
        + "\n\n"
...
def parse_fgf_json(raw: str, patch_menu: list[str] | None = None) -> dict:
    try:
    ...
    patch_menu = patch_menu or SCENARIO_S1_BAIL["patch_menu"]
    allowed_patch = set(patch_menu)
    ...
    if parsed["patch_attempt_type"] not in allowed_patch:
        ...
        if candidate in allowed_patch:
            ...
            parsed["patch_attempt_type"] = patch_menu[0]
...
def run(args: argparse.Namespace) -> None:
    ...
    with tracer.start_as_current_span("cgf_fgf.run") as run_span:
        span_context = run_span.get_span_context()
        ...
        run_span.set_attribute(SpanAttributes.SESSION_ID, run_id)
        run_span.set_attribute(
        ...
        with open(out_path, "w", encoding="utf-8") as f:
            for round_n in range(args.rounds):
                with tracer.start_as_current_span("cgf_fgf.round") as round_span:
                    round_span.set_attribute(
                        SpanAttributes.OPENINFERENCE_SPAN_KIND,
                        OpenInferenceSpanKindValues.CHAIN.value,
                    )
                    round_span.set_attribute(SpanAttributes.SESSION_ID, run_id)
                    round_span.set_attribute(
                        SpanAttributes.INPUT_VALUE,
                        json.dumps({"round": round_n, "prior_fgf": prior_fgf}),
                    )
                    ...
                    round_span.set_status(Status(StatusCode.OK))

        run_span.set_attribute(
            SpanAttributes.OUTPUT_VALUE,
            json.dumps({"out_path": out_path, "records_written": records_written}),
        )
